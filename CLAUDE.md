...
| `PROMOTE_GENRE_LOOKUP` | `1` (on) | Let a finished download resolve an unknown artist's genre from MusicBrainz when neither the curated map nor your library knows it. Set `0` to keep promotion fully offline (new artists then land untagged, as before) |
...
**iPod-Prep tables** (all created in `db_init`, per the HANDOFF §7): `prep_jobs` (job queue/history; `type` = `audit`\|`tags`\|`unify`\|`convert`; done-job summary counts are stored as JSON in the `error` column), `prep_changes` (tag-edit rollback log), `library_tracks` (scanned library index), `playlists` (playlist specs), `pipeline_state` (latest completed summary **per step type** — the durable source of truth for the Dashboard/stepper), `review_dismissed` (`(kind, key)` → a report row the user judged a false positive; `kind` is `albumartist`\|`genrealign`, `key` is the album folder — see the dismiss endpoint), `crosscheck_state` (`artist_key` → cached external genre result + `dismissed` flag, for the genre cross-check's incremental coverage; the `dismissed` column is added by a `db_init` migration for pre-existing tables). M1 uses only `prep_jobs`; the rest are seeded ahead for later milestones.
...
| POST/DELETE | `/api/prep/review/dismiss` | **Hide a review's false positives for good** (`{kind, keys:[folder]}`; DELETE `?kind=` restores). Backed by `review_dismissed`. Serves the album-artist + album-outlier reports, which re-derive from `library_tracks` on every scan and would otherwise re-offer the same classical/DJ albums forever — unchecking a row only skips it for *that* apply. Writes no tags: it's a judgement about the suggestion, not the music |
...
**Edit vs. Re-curate (no more Regenerate):** every card has **Edit** (`editPlaylist` → opens the playlist's resolved tracks in the same staging list; `/{id}/tracks` now returns `path` so the edit can freeze on it). Rename/reorder/add/remove/re-target, then Save → **PUT `/{id}`**. An *untouched* save keeps the original spec (a rules playlist stays live and keeps re-matching); *any* edit freezes it (`frozen`+`ai_paths`), so `PUT`'s guard had to accept a frozen spec on a `smart` row (the old `type=='smart' and not rules` check rejected exactly that). **Regenerate was removed** — `regenerate_all_auto()` already rewrites every `auto_refresh` playlist's `.m3u` after any library/mirror-changing job and nightly, and Edit→Save covers "rewrite it now", so a manual ↻ button was redundant. **Re-curate** stays, `ai` only: `POST /{id}/recurate` re-runs the two-stage curation for the saved prompt (fresh `ai_paths`), keeping name + targets. **Staging now carries the targets control** (`#stageTargetLibrary`/`#stageTargetIpod`), so both create and edit set targets there; a frozen card summarizes as "Hand-curated"/"Hand-picked" rather than its stale rules.
...
7. **Promote** (`_promote_download`, `AUTO_PROMOTE` on + `MUSIC_DIR` set): each finished file is **moved** into `MUSIC_DIR` **organized as `<Artist>/<Album>/<file>`** (staging only has `<Album>/<file>`). The artist is the **primary** album artist (`_primary_artist` — first of a comma/semicolon list, so a track featuring guests still files under the band), and it's **merged into an existing artist folder** when one matches loosely (`_normalize_artist` — case-insensitive, ignoring a leading "The", so "The Black Eyed Peas" lands in an existing "Black Eyed Peas"); album falls back to the staging folder, both to "Unknown". YouTube supplies neither album-artist nor genre, so `_fill_missing_tags` fills them in place before mirroring: **album-artist** ← the primary artist (when empty); **genre** ← the curated `artist_genres.json` map, else the **dominant genre of that artist's existing library tracks** (`_build_genre_by_artist`), else **MusicBrainz** (`tagtools.musicbrainz_genres`, the same resolver the genre review uses), else left empty for the Complete-genres step. The MusicBrainz step exists because the two local sources only cover artists you *already have* — the curated map is 78 artists — so genuinely new music landed untagged and needed the genre tools run by hand. It's a **last resort**: a file that already has a genre, an artist in the curated map, or one the library knows never touches the network. Bounded because each call costs ~2 s (MusicBrainz asks ≤1 req/sec): results are **cached per promoted batch** (a 12-track album by one new artist is ONE lookup, and a miss is cached too so an unresolvable artist isn't retried per track) and capped at `_PROMOTE_MB_CAP` (25) lookups per batch. Trimmed to `_MB_GENRE_LIMIT` (2) — MusicBrainz returns up to 7 (Chappell Roan), which is noise to write unattended. `PROMOTE_GENRE_LOOKUP=0` turns it off. Then it's **copied** to `IPOD_DIR` in the same tree (m4a is copy-only — already AAC), and **indexed** via `prep._upsert_tracks` with the **resolved MUSIC_DIR path** (required — else `converter.mirror_path` raises and iPod playlists silently drop the track). Then `playlists.regenerate_all_auto()` refreshes auto playlists, a `promoted` WS event fires, and `prep.schedule_autoprocess()` is (re)armed (no-op unless the auto-process toggle is on). Isolated from the worker's error path — a promotion failure never flips a successful download to `error` (the file stays safe in staging).

**Files browser repoint:** when promotion is active, `GET/DELETE /api/files` operate on `MUSIC_DIR` (the library), not `DOWNLOADS_DIR`. `delete_file` cascades — removes the iPod mirror copies (`converter.mirror_path`), deletes the `library_tracks` rows, and regenerates auto playlists, all **once per call** across the whole batch. It then **sweeps the mirror folders it touched** (`prep.sweep_mirror_dirs`): `mirror_path` only finds the twin the source would produce *today*, so a mirror that has drifted — an album re-added in another format, leaving the old `.m4a` beside the new `.mp3` — keeps a stale file the mapping can't see, and deleting the source would strand it forever (measured: deleting `01 song.mp3` left `01 song.m4a` behind). The sweep is **scoped to the touched folders**, not a full orphan scan: that walks both roots (O(library), minutes over a network mount) and would surface orphans unrelated to the delete. It removes only files with **no source at all** — `prep.mirror_has_source` reuses the orphan scan's keep-rule (same relative path, or a lossless source with the same stem), so a live transcoded twin is never touched. The response returns `mirror_orphans_removed`; the Files page mentions it only when it's non-zero. It collects the affected files *before* deleting (`rglob` can't see a tree that's already gone) and de-dupes, so selecting a folder and something inside it counts once.
...
- **Clean** (`run_clean`) normalizes genres (`normalize_genre`) and fills missing album-artist (`fill_album_artist`), writing files **in place**. **It works from the Audit's `needs_clean` flags, not a fresh walk** (`prep._clean_targets` → `only_paths`): Clean used to re-read the entire library to rediscover what the Audit that just ran had already decided — on a clean 12k library that's 12,163 reads to change nothing. The verdict comes from **`tagtools.clean_plan`, which Audit and Clean both call** — that shared function is load-bearing: if Audit's idea of "needs work" ever drifted from Clean's, Clean would silently skip files it should fix (verified identical to the old inline logic across 1,500 real files). `_clean_targets` returns **None, not []**, when no row under the root has a non-NULL `needs_clean` — i.e. no Audit has run since the column existed — and None means "walk everything", preserving old behaviour for an unaudited library; `[]` would mean "audited, nothing to do", a very different claim. Flags are cleared after a successful run, and the summary carries `scanned_all` so the UI can say "1 flagged by audit" rather than implying it checked the lot. Before each file is touched, `record_cb` durably persists the pre-image to `prep_changes` (so a crash mid-run still leaves it rollback-able). Rollback restores those pre-images; an empty pre-image deletes the tag (faithful restore of an originally-absent value).
...
**Visual language borrowed from Ray's `minset` project** (`~/Documents/minset`): pure-black surfaces (`--bg #000`, `--surface #0e0e0f`), **hairline borders** (white at 12%/7% alpha via `--border`/`--border-soft`, not solid grey), soft 16px radii (`--radius`), the SF Pro system stack with antialiasing + `0.01em` body tracking and `-0.02em` on large display titles, iOS system state colors (`--success #34c759`, `--danger #ff453a`, `--info #5ea2ff`), and the Apple ease `cubic-bezier(.16,1,.3,1)` (`--ease`) on buttons/nav. The **red `--accent #ff0033` is kept** — it's the brand (the equalizer logo); minset's blue was NOT adopted. So: minset's typography + surface treatment, Music Monster's identity.
...
- **Optimize** (`panel-convert`) is a **3-step stepper** — Clean tags (1) → Analyze BPM (2) → Convert (3) — plus a **Mirror maintenance** card (the read-only DRM + orphan scans, which aren't steps: Convert only ever *adds*, these find what it can't). There is no Audit step and no Complete-genres step here; both live on the Audit page. Each `.step` card carries the tool's existing controls; `updatePrepSteps()` reads `/api/prep/pipeline` to set per-step status + the `.done` state (its `set()` is null-safe, so it also drives `#stepGenresStatus` over on the Audit page).
  ...
  - **Apple Music card (`#appleMusicCard`)** — sits right after Convert, since it's where the mirror hands off to a player; **not** a numbered step (steps are jobs *this app* runs) and not in Mirror maintenance (that card is read-only checks *on* the mirror). Music Monster **cannot touch Apple Music** — it's a container on the server, Music is an app on the Mac, and Music's library is a private local DB with no network API. All it can do is hand you a script. The script is *static* (no job, queue, DB, or change-tracking) and does three things: `add POSIX file "<mirror>"` picks up new files (verified idempotent on a real 12k library — re-running added nothing); a **per-track `refresh` loop** re-reads tags from disk; and it then **offers to remove entries whose files are gone**, behind a confirm dialog defaulting to *Keep them*.

**Why step 3 exists:** Convert only ever *adds*, so replacing an album (rename, or mp3→m4a) orphans the old mirror files. Pruning them leaves Apple Music holding dead entries — and the iPod keeps the audio it already synced, because a device only drops a track when the **track** leaves the library, not when its file vanishes. That's the "songs on my iPod that aren't in the folder" symptom, and it recurs on every library edit. Order matters: prune mirror orphans → remove dead entries → sync.

**Two traps, both hit for real:**
- **`location` returns `missing value` for a dead track — it does NOT raise.** A `try/on error` around it never fires, so a dead check written that way silently finds nothing. The check must test the *value*. (`whose location is missing value` also matches nothing — the `whose` clause won't compare against `missing value` — which is what makes the per-track loop necessary.)
- **Unmounted drive = every track looks dead.** Without the `System Events` guard (folder exists *and* is non-empty) step 3 would offer to delete the entire library. Verified: pointed at an unmounted path it aborts before touching Music. **The loop is not a style choice:** `refresh (every file track ...)` throws **error 9044** the moment it touches a track whose file is gone, killing the whole call, and `whose location is missing value` can't pre-filter them because Music *raises* on `location` instead of returning missing value — so each `refresh` needs its own `try`. Costs ~17 ms/track (≈3.5 min for 12k), hence the "not stuck" note in the script and the card copy. `refresh` is the whole point — it's what the old delete-the-library-and-reimport dance was working around, and it keeps play counts/ratings/playlists that a wipe destroys. Verified against a real 12k library: a track's genre changed on disk stayed stale in Music until `refresh`, then updated; the plural specifier works despite the dictionary declaring a singular `file track` param. **The Mac's mirror path is NOT the server's `IPOD_DIR`** (same files, different mount), so `#amMacPath` asks for it and keeps it in `localStorage` (`mm_am_mac_path`) — it's a display string used only to build the shown script; the server never reads it. The script block is **collapsed** (`#amScriptDetails`) — it's ~50 lines and gets copied far more often than read. `copyAppleScript` is **three tiers**, because collapsing and `http://` interact: (1) `navigator.clipboard`, which needs a **secure context** a self-hosted `http://` origin doesn't have, so it's usually skipped; (2) `document.execCommand('copy')` via an off-screen textarea — deprecated, but it works on plain http **and doesn't need the `<pre>` rendered**, which the old select-the-`<pre>` fallback did (you can't select what's inside a closed `<details>`); (3) last resort, force the disclosure open *then* select, so the "press ⌘C" hint can't be a lie. Verified with a real click (not `.click()` — `execCommand` needs genuine user activation, which scripted clicks lack): tier 2 returns true, the block stays shut, and `pbpaste` shows the full 51-line script on the system clipboard with UTF-8 intact.
