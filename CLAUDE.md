...
**Staging / review before saving:** both creators resolve first and open an editable **staging list** (`#plStaging`) instead of saving straight away — AI's "Generate" → `POST /ai/preview`, smart's "Preview" → `POST /preview` (both return the full track list). `_openStaging({type,name,spec,targets,tracks})` renders it: an editable name, an "add track" box (debounced `GET /playlists/search` → `stageAdd`), and the numbered list with a ✕ per row (`stageRemove`). `renderStageList()` re-renders **only the list** so editing never steals focus from the search box. `saveStaging()` POSTs to `/api/playlists` with `{name, type, spec, targets}` — and **only freezes if the user actually edited**: `spec.frozen=true` + `ai_paths=[final paths]`. `_matched_for_spec` checks `frozen` **first**, so a regenerate replays the hand-curated set instead of re-matching (which would undo the edits); an untouched list keeps its normal behavior (smart rules / a completionist set still pick up new music). Frozen cards show a `.pl-frozen` "hand-curated" badge. The smart "Save Playlist" button remains the direct, unreviewed path.
