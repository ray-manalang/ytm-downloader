...
**Process new additions (chained pipeline):** `/api/prep/process` doesn't add a new job type — it enqueues the first selected step as a normal typed job whose `settings.chain` holds the remaining steps (`chain_output`/`chain_downsample`/`chain_auto` ride along). On each successful `done`, the worker enqueues the next step (cancel/error stops the chain). So each step reuses its existing engine/summary/rollback and updates the stepper naturally, and the user sees the steps run as ordinary cards in sequence. `_valid_chain` drops steps that can't run (Clean on a read-only mount, Analyze without librosa, Convert with no output dir). **Auto-after-downloads:** when `process/config.enabled`, `main._promote_download` calls `prep.schedule_autoprocess()`, a 45 s debounce timer that fires once a download *batch* settles; `_run_autoprocess` defers (re-arms) while any prep job is active so it never stacks. Complete-genres is intentionally excluded (needs manual review). WebSocket message types: `prep_added`, `prep_progress` (`done`/`total`/`current_file`/`action`), `prep_status` (`running`/`done`/`error`/`cancelled`, with a `summary` counts dict on done), `prep_removed`.
...
Single-file, no build step. **Left-sidebar app shell** (`.app` grid: `.sidebar` + `.content` with a sticky `.topbar`) — not a top tab bar. Nav is grouped: **Dashboard** (default landing) · **Activity** · Download (**YouTube Music** / **Add URLs**) · Prepare (**Prepare library**) · Organize (**Playlists** / **Files**) · Settings (**Setup**). `switchTab(name)` (aliased `navigate`) toggles `.nav-item.active` + `.panel.active`, sets the topbar title, and calls the page's loader. Internal panel ids are unchanged (`panel-convert` is the Prepare page), so the JS keeps working. All nav icons are inline `currentColor` line-SVGs (muted → accent on active); YouTube Music keeps the filled equalizer brand mark.

**Activity page (`panel-activity`):** consolidates *all* jobs/processes — the old Queue + History pages and the Prepare page's Jobs list are gone. `renderActivity()` reads the `downloads` and `prepJobs` state maps and splits by status into three sections: **Running** (active downloads via `cardHTML` + active prep jobs via `prepCardHTML`, newest first), **Downloads** (finished downloads), **Library jobs** (finished prep jobs). `renderAll()` and `renderPrepJobs()` both just delegate to `renderActivity()`, so every existing WS/init call site keeps working and in-place progress updates (`updateCardInPlace`/`updatePrepCardInPlace`, by `card-<id>`/`prepcard-<id>`) still find their card wherever it landed. The `activityBadge` (nav) + `runningCount` show the active total. **To stay compact, Running items keep full progress cards but finished items render as dense one-line rows** (`dlRowHTML`/`jobRowHTML`). A sticky toolbar has a **view segment** (`actView`: all/downloads/jobs — persisted; hides the other sections) + **status segment** (`actStatus`: any/done/error) that both narrow the display and scope **Remove all** (`removeAllActivity` deletes exactly the finished items matching the filter — never running ones).

**Setup page (`panel-setup`):** owns the **Music library folder** input (`#libDir`, the single source of truth the Prepare steps read cross-panel) and the **YouTube Music connection** (OAuth/browser setup flow + connected bar with Disconnect). `applyYtmConnection()` drives both Setup (connect flow vs. connected bar) and the YouTube Music page (browsing vs. a "Connect in Setup →" prompt) off one `/api/ytm/status`; `refreshYtm()` is the shared refresh called by connect/disconnect and both page loaders. The Prepare page shows a read-only `#prepLibDisplay` mirror of `#libDir` (kept in sync by `onLibDirChange`) with a "Change in Setup →" link.

**Dashboard genre analytic:** `genreAnalyticHTML()` renders the top 12 genres from the latest audit's `genre_distribution` as horizontal accent bars with count + share% (`(none)` filtered out).
